...
ipcRenderer.on('auth-data', (event, data) => {
  if (!data) return;
  if (data.nickname) myChar.name = data.nickname;
  if (data.parts) myChar.parts = data.parts;
  document.getElementById('settingsNicknameVal').textContent = myChar.name;
...
function tryAutoReconnect() {
  try {
    ...
    const store = new Store({ name: 'hellowee-auth' });
    const lastRoom = store.get('lastRoom');
    if (lastRoom && lastRoom.code && lastRoom.roomName) {
      connectSocket();
      socket.on('connect', () => {
        socket.emit('join-room', {
          code: lastRoom.code, roomName: lastRoom.roomName,
          name: myChar.name, parts: myChar.parts,
        }, (res) => {
          if (res.success) {
...
document.addEventListener('mouseup',(e)=>{
  ...
  if(!isDragging){
    ...
    if(dragTarget==='self'){
      // 튜토리얼 액션 체크
      if(tutorial&&tutorial.isActive){if(tutorial.triggerAction('click-character'))return;}
      if(lastClickTarget==='self'&&(now-lastClickTime)<DBLCLICK_TIME){
        if(tutorial&&tutorial.isActive){tutorial.triggerAction('dblclick-character');}
        else{openPhone('home');}
        lastClickTarget=null;
      }else{
        const action=RANDOM_ACTIONS[Math.floor(Math.random()*RANDOM_ACTIONS.length)];
        myChar.state=action;setTimeout(()=>{if(myChar.state===action)myChar.state='idle';},1500);
        showEmptyBubble(()=>({x:myChar.x,y:myChar.y}),'chat',()=>openQuickChat(myChar.x+CHAR_SIZE/2,myChar.y-50,'chat'));
        lastClickTarget='self';lastClickTime=now;
      }
    }else{
      const tid=dragTarget,c=otherChars[tid];
      if(c)showEmptyBubble(()=>({x:c.x,y:c.y}),'whisper',()=>openQuickChat(c.x+CHAR_SIZE/2,c.y-50,'whisper',tid));
    }
  }else{
    const dt=now-dragStartTime,dist=mouseDownPos?Math.hypot(e.clientX-mouseDownPos.x,e.clientY-mouseDownPos.y):0,speed=dist/(dt||1)*1000;
    if(tutorial&&tutorial.isActive){tutorial.triggerAction('drag-character');}
    if(dragTarget==='self'){myChar.isGrabbed=false;if(speed>THROW_SPEED_THRESHOLD){playSound('throw');myChar.state='thrown';myChar.vx=(e.clientX-mouseDownPos.x)*0.15;myChar.vy=Math.min((e.clientY-mouseDownPos.y)*0.15,-3);}else{myChar.state='fall';myChar.vy=1;myChar.vx=0;}}
    else if(socket){if(speed>THROW_SPEED_THRESHOLD){playSound('throw');socket.emit('throw-other',{targetId:dragTarget,vx:(e.clientX-mouseDownPos.x)*0.1,vy:-3});}else socket.emit('drop-other',{targetId:dragTarget});}
  }
  ipcRenderer.send('set-ignore-mouse',true);dragTarget=null;mouseDownPos=null;isDragging=false;
});
...
function openQuickChat(x,y,mode,tid){
  quickChatMode=mode;quickWhisperTarget=tid||null;
  quickChatLabel.textContent=mode==='whisper'?`🤫 ${otherChars[tid]?.name||'???'}`:'💬 전체';
  quickChat.style.left=Math.min(x,window.innerWidth-260)+'px';
  quickChat.style.top=Math.max(y,30)+'px';
  quickChat.classList.add('visible');
  // 이모지 버튼 추가
  let emojiBar=quickChat.querySelector('.emoji-bar');
  if(!emojiBar){
    emojiBar=document.createElement('div');
    emojiBar.className='emoji-bar';
    emojiBar.style.cssText='display:flex;gap:2px;flex-wrap:wrap;margin-top:4px;';
    EMOJIS.forEach(em=>{
      const btn=document.createElement('button');
      btn.textContent=em;
      btn.style.cssText='background:none;border:1px solid #2a2a35;border-radius:8px;padding:4px 6px;cursor:pointer;font-size:14px;';
      btn.addEventListener('click',()=>{quickChatInput.value+=em;quickChatInput.focus();});
      emojiBar.appendChild(btn);
    });
    quickChat.appendChild(emojiBar);
  }
  quickChatInput.value='';quickChatInput.focus();
...
const pages=document.querySelectorAll('.phone-page'),homeIcons=document.querySelectorAll('.home-icon');
...
lobbyCopyCode.addEventListener('click',()=>{
  ...
  const text=`🐾 Hellowee 방 초대!\n방 이름: ${lobbyRoomTitle.textContent}\n방 코드: ${currentRoom}`;
  navigator.clipboard.writeText(text);
  lobbyCopyCode.textContent='✅ 복사됨';setTimeout(()=>{lobbyCopyCode.textContent='📋 초대링크';},1500);
});

function saveLastRoom(code,roomName){try{const Store=require('electron-store');const s=new Store({name:'hellowee-auth'});s.set('lastRoom',{code,roomName});}catch(e){}}

lobbyCreate.addEventListener('click',()=>{
  const name=lobbyMyName.value.trim()||myChar.name;const roomName=lobbyRoomName.value.trim()||'Hellowee Room';
  myChar.name=name;
  if(socket)socket.disconnect();
  connectSocket();
  socket.on('connect',()=>{socket.emit('create-room',{roomName,name,parts:myChar.parts},(res)=>{if(res.success){currentRoom=res.code;saveLastRoom(res.code,roomName);updateLobbyUI();switchPage('home');lobbyError.textContent='';}});});
});

lobbyJoin.addEventListener('click',()=>{
  const code=lobbyCode.value.trim().toUpperCase(),roomName=lobbyJoinRoomName.value.trim(),name=lobbyJoinName.value.trim()||myChar.name;
  if(!code||!roomName){lobbyError.textContent='방 이름과 코드를 모두 입력해주세요';return;}
  myChar.name=name;
  if(socket)socket.disconnect();
  connectSocket();
  socket.on('connect',()=>{socket.emit('join-room',{code,roomName,name,parts:myChar.parts},(res)=>{if(res.success){currentRoom=res.code;saveLastRoom(res.code,roomName);updateLobbyUI();switchPage('home');lobbyError.textContent='';}else{lobbyError.textContent=res.error;}});});
});

function updateLobbyUI(){
  lobbyRoomCode.textContent=currentRoom||'---';lobbyMembers.innerHTML='';
  if(currentRoom){
    ...
    Object.entries(otherChars).forEach(([id,c])=>{
      ...
      let statusTag=c.status?` <span style="font-size:9px;color:#888">${c.status}</span>`:'';
      let html=`<div class="lobby-member-left"><span class="lobby-member-dot"></span> ${c.name}${statusTag}</div>`;
      if(isRoomOwner){
        html+=`<div style="display:flex;gap:4px;"><button class="lobby-kick-btn" data-kick-id="${id}">강퇴</button><button class="lobby-kick-btn" data-transfer-id="${id}" style="border-color:#6C8EBF;color:#6C8EBF;">위임</button></div>`;
      }
      div.innerHTML=html;lobbyMembers.appendChild(div);
    });
    lobbyMembers.querySelectorAll('.lobby-kick-btn[data-kick-id]').forEach(btn=>{btn.addEventListener('click',(e)=>{e.stopPropagation();const tid=btn.dataset.kickId;if(socket&&confirm(`${otherChars[tid]?.name}님을 강퇴할까요?`))socket.emit('kick-user',{targetId:tid});});});
    lobbyMembers.querySelectorAll('.lobby-kick-btn[data-transfer-id]').forEach(btn=>{btn.addEventListener('click',(e)=>{e.stopPropagation();const tid=btn.dataset.transferId;if(socket&&confirm(`${otherChars[tid]?.name}님에게 방장을 넘길까요?`))socket.emit('transfer-owner',{targetId:tid});});});
  }
}
...
settingsLeave.addEventListener('click',()=>{
  ...
  try{const Store=require('electron-store');const s=new Store({name:'hellowee-auth'});s.delete('lastRoom');}catch(e){}
...
if(statusContainer){
  STATUS_LIST.forEach(s=>{
    const btn=document.createElement('button');
    btn.textContent=s;
    btn.style.cssText='padding:5px 10px;border-radius:99px;border:1px solid #2a2a35;background:#1a1a22;color:#aaa;font-size:10px;cursor:pointer;margin:2px;';
    btn.addEventListener('click',()=>{
      myChar.status=s;
      if(socket)socket.emit('update-status',{status:s});
      statusContainer.querySelectorAll('button').forEach(b=>b.style.borderColor='#2a2a35');
      btn.style.borderColor='#6C8EBF';
    });
...
function startTutorial(){
  tutorial=new Tutorial({
    onStep:(step,i)=>{playSound('click');},
    onFinish:()=>{tutorial=null;openPhone('lobby');},
    onSkip:()=>{tutorial=null;openPhone('lobby');},
  });
  tutorial.start();
}
...
// ════ 나가기 애니메이션 ════
function animateLeave(id){const c=otherChars[id];if(!c)return;c.state='wave';createTrackedBubble(()=>({x:c.x,y:c.y}),'👋 안녕~','chat');playSound('leave');setTimeout(()=>{if(c.el){c.el.style.transition='opacity 0.5s';c.el.style.opacity='0';}if(c.nameEl){c.nameEl.style.transition='opacity 0.5s';c.nameEl.style.opacity='0';}setTimeout(()=>{if(c.el)c.el.remove();if(c.nameEl)c.nameEl.remove();delete otherChars[id];updateLobbyUI();},500);},WAVE_LEAVE_DURATION);}
...
function connectSocket(){socket=io(SERVER_URL);
  ...
  socket.on('user-joined',({id,name,parts,status})=>{
    if(otherChars[id])return;playSound('join');
    const el=document.createElement('canvas');el.width=CHAR_SIZE;el.height=CHAR_SIZE;el.className='character';el.style.width=CHAR_SIZE+'px';el.style.height=CHAR_SIZE+'px';document.body.appendChild(el);
    ...
    otherChars[id]={x:Math.random()*(window.innerWidth-CHAR_SIZE),y:FLOOR_Y,vx:0,vy:0,state:'idle',dir:1,frame:0,frameTimer:0,name,status:status||'',parts:parts||{},el,ctx:el.getContext('2d'),nameEl};
  ...
  socket.on('user-status-updated',({id,status})=>{if(otherChars[id]){otherChars[id].status=status;updateLobbyUI();}});
  socket.on('user-moved',({id,x,y,state,dir,frame})=>{if(otherChars[id])Object.assign(otherChars[id],{x,y,state,dir,frame});});
  socket.on('user-leaving',({id})=>{animateLeave(id);});
  ...
  socket.on('owner-changed',({newOwnerId,newOwnerName})=>{isRoomOwner=(newOwnerId===socket.id);updateLobbyUI();createTrackedBubble(()=>({x:myChar.x,y:myChar.y}),`👑 ${newOwnerName}님이 새 방장!`,'chat');});
  ...
  socket.on('force-grabbed',()=>{myChar.forcedGrab=true;myChar.state='grabbed';myChar.vx=0;myChar.vy=0;playSound('grab');});
  ...
  socket.on('force-thrown',({vx,vy})=>{myChar.forcedGrab=false;myChar.attentionGrab=false;myChar.state='thrown';myChar.vx=vx;myChar.vy=vy;playSound('throw');});
