...
lobbyCreate.addEventListener('click',()=>{const name=lobbyMyName.value.trim()||myChar.name;const roomName=lobbyRoomName.value.trim()||'Hellowee Room';myChar.name=name;if(socket)socket.disconnect();connectSocket();socket.on('connect',()=>{socket.emit('create-room',{roomName,name,parts:myChar.parts},(res)=>{if(res.success){currentRoom=res.code;saveLastRoom(res.code,roomName);updateLobbyUI();switchPage('home');lobbyError.textContent='';}});});});
lobbyJoin.addEventListener('click',()=>{const code=lobbyCode.value.trim().toUpperCase(),roomName=lobbyJoinRoomName.value.trim(),name=lobbyJoinName.value.trim()||myChar.name;if(!code||!roomName){lobbyError.textContent='방 이름과 코드를 모두 입력해주세요';return;}myChar.name=name;if(socket)socket.disconnect();connectSocket();socket.on('connect',()=>{socket.emit('join-room',{code,roomName,name,parts:myChar.parts},(res)=>{if(res.success){currentRoom=res.code;saveLastRoom(res.code,roomName);updateLobbyUI();switchPage('home');lobbyError.textContent='';}else{lobbyError.textContent=res.error;}});});});
function updateLobbyUI(){lobbyRoomCode.textContent=currentRoom||'---';lobbyMembers.innerHTML='';if(currentRoom){const me=document.createElement('div');me.className='lobby-member';me.innerHTML=`<div class="lobby-member-left"><span class="lobby-member-dot"></span> ${myChar.name} (나)${isRoomOwner?' 👑':''}</div>`;lobbyMembers.appendChild(me);Object.entries(otherChars).forEach(([id,c])=>{const div=document.createElement('div');div.className='lobby-member';let statusTag=c.status?` <span style="font-size:9px;color:#888">${c.status}</span>`:'';let html=`<div class="lobby-member-left"><span class="lobby-member-dot"></span> ${c.name}${statusTag}</div>`;if(isRoomOwner){html+=`<div style="display:flex;gap:4px;"><button class="lobby-kick-btn" data-kick-id="${id}">강퇴</button><button class="lobby-kick-btn" data-transfer-id="${id}" style="border-color:#6C8EBF;color:#6C8EBF;">위임</button></div>`;}div.innerHTML=html;lobbyMembers.appendChild(div);});lobbyMembers.querySelectorAll('.lobby-kick-btn[data-kick-id]').forEach(btn=>{btn.addEventListener('click',(e)=>{e.stopPropagation();const tid=btn.dataset.kickId;if(socket&&confirm(`${otherChars[tid]?.name}님을 강퇴할까요?`))socket.emit('kick-user',{targetId:tid});});});lobbyMembers.querySelectorAll('.lobby-kick-btn[data-transfer-id]').forEach(btn=>{btn.addEventListener('click',(e)=>{e.stopPropagation();const tid=btn.dataset.transferId;if(socket&&confirm(`${otherChars[tid]?.name}님에게 방장을 넘길까요?`))socket.emit('transfer-owner',{targetId:tid});});});}}
...
settingsLeave.addEventListener('click',()=>{if(socket)socket.disconnect();currentRoom=null;isRoomOwner=false;try{const Store=require('electron-store');const s=new Store({name:'hellowee-auth'});s.delete('lastRoom');}catch(e){}Object.keys(otherChars).forEach(id=>{otherChars[id].el.remove();otherChars[id].nameEl.remove();delete otherChars[id];});chatHistory=[];whisperHistory=[];updateLobbyUI();switchPage('lobby');});
...
let tutorial=null;
function startTutorial(){tutorial=new Tutorial({onStep:(step,i)=>{playSound('click');},onFinish:()=>{tutorial=null;openPhone('lobby');},onSkip:()=>{tutorial=null;openPhone('lobby');},});tutorial.start();}
const activeBubbles=[];
...
function updateBubbles(){activeBubbles.forEach(b=>{if(!b.el.parentElement)return;const p=b.charGetter();b.el.style.left=(p.x+DISPLAY_SIZE/2)+'px';b.el.style.top=(p.y-40)+'px';});if(activeEmptyBubble?.updatePos)activeEmptyBubble.updatePos();}
...
let lastTime=0;
function loop(ts){const dt=ts-lastTime;lastTime=ts;if(!myChar.isGrabbed&&!myChar.forcedGrab){if(myChar.y<FLOOR_Y){myChar.vy+=GRAVITY;if(myChar.state!=='thrown')myChar.state='fall';}else{myChar.y=FLOOR_Y;myChar.vy=0;if(myChar.state==='fall'||myChar.state==='thrown'){playSound('land');myChar.state='land';myChar.vx=0;setTimeout(()=>{myChar.state='idle';randomWalk();},300);}if(!updateAttentionMode()&&!RANDOM_ACTIONS.includes(myChar.state)){walkTimer--;if(walkTimer<=0&&myChar.state!=='land')randomWalk();}myChar.x+=myChar.vx;checkProximityInteract();}myChar.y+=myChar.vy;myChar.x=Math.max(0,Math.min(window.innerWidth-DISPLAY_SIZE,myChar.x));myChar.y=Math.min(FLOOR_Y,myChar.y);}else if(myChar.attentionGrab){myChar.x=mousePos.x-DISPLAY_SIZE/2;myChar.y=mousePos.y-DISPLAY_SIZE/2;}myChar.frameTimer+=dt;if(myChar.frameTimer>1000/FPS){myChar.frameTimer=0;myChar.frame=(myChar.frame+1)%(ANIM_FRAMES[myChar.state]||2);}renderChar(myCtx,myEl,myChar);updateAlertPos();updateBubbles();Object.values(otherChars).forEach(c=>{if(c.ctx){renderChar(c.ctx,c.el,c);c.nameEl.style.left=(c.x+DISPLAY_SIZE/2)+'px';c.nameEl.style.top=(c.y-18)+'px';}});if(socket&&socket.connected){socket.volatile.emit('move',{x:myChar.x,y:myChar.y,state:myChar.state,dir:myChar.dir,frame:myChar.frame});}requestAnimationFrame(loop);}
...
function connectSocket(){socket=io(SERVER_URL);
  ...
  socket.on('user-joined',({id,name,parts,status})=>{if(otherChars[id])return;playSound('join');const el=document.createElement('canvas');el.width=CHAR_SIZE;el.height=CHAR_SIZE;el.className='character';el.style.width=DISPLAY_SIZE+'px';el.style.height=DISPLAY_SIZE+'px';document.body.appendChild(el);const nameEl=document.createElement('div');nameEl.className='nameplate';nameEl.textContent=name;document.body.appendChild(nameEl);otherChars[id]={x:Math.random()*(window.innerWidth-DISPLAY_SIZE),y:FLOOR_Y,vx:0,vy:0,state:'idle',dir:1,frame:0,frameTimer:0,name,status:status||'',parts:parts||{},el,ctx:el.getContext('2d'),nameEl};el.addEventListener('mousedown',(e)=>{e.stopPropagation();mouseDownPos={x:e.clientX,y:e.clientY};dragTarget=id;dragOffset.x=e.clientX-otherChars[id].x;dragOffset.y=e.clientY-otherChars[id].y;isDragging=false;dragStartTime=Date.now();ipcRenderer.send('set-ignore-mouse',false);});updateLobbyUI();});
